...
if (!class_exists('ALRespStream')) :

	class ALStream extends ALCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new ALRespStream($request);
			if ($request->isAPICall()) {
				$stream = new ALHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class ALRespStream extends ALStream {
	public $bvboundry;
...
class ALHttpStream extends ALStream {
	var $user_agent = 'ALHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
	...
	public function getResponse() {
		...
		while (!feof($this->conn)) {
			...
			if (1 == $state) {
				if (!ALHelper::safePregMatch('/HTTP\/(\\d\\.\\d)\\s*(\\d+)\\s*(.*)/', $line, $m)) {
				...
				if (!ALHelper::safePregMatch('/([^:]+):\\s*(.*)/', $line, $m)) {
