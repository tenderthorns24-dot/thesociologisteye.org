...
if (!class_exists('ALManageCallback')) :
class ALManageCallback extends ALCallbackBase {
	public $settings;
	public $skin;
	public $bvinfo;
		...
		if (!empty($valid_updates)) {
			if (class_exists('Language_Pack_Upgrader')) {
				if ($has_bv_skin) {
					...
					$skin = new ALUpgraderSkin("upgrade_translations");
					$this->skin = $skin;
				} else {
	...
	function upgradePlugins($plugins, $has_bv_skin = false, $bv_bulk_upgrade = false) {
		...
		if (class_exists('Plugin_Upgrader')) {
			if ($has_bv_skin) {
				require_once( "bv_upgrader_skin.php" );
				$skin = new ALUpgraderSkin("plugin_upgrade", $plugins_by_name);
				$this->skin = $skin;
			} else {
	...
	function upgradeThemes($themes, $has_bv_skin = false, $bv_bulk_upgrade = false) {
		...
		if (class_exists('Theme_Upgrader')) {
			if ($has_bv_skin) {
				require_once( "bv_upgrader_skin.php" );
				$skin = new ALUpgraderSkin("theme_upgrade");
				$this->skin = $skin;
			} else {
	...
	function installPackage($type, $params, $has_bv_skin = false) {
		...
		if (ALHelper::safePregMatch($valid_domain_regex, $params['package']) !== 1) {
			return array('status' => "Error", 'message' => "Invalid package domain");
		}
		if ($has_bv_skin) {
			require_once( "bv_upgrader_skin.php" );
			$skin = new ALUpgraderSkin("installer", array(), $params['package']);
			$this->skin = $skin;
		} else {
