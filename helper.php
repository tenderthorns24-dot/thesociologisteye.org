...
if (!class_exists('ALHelper')) :
	class ALHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!ALWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = ALWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					ALWPFileSystem::getInstance()->putContents($fname, $modified_content,
							ALWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
